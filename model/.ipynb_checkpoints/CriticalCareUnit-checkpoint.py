...
class Scenario:
    ...
    def init_sampling(self):
        ...
        self.seeds = rng_streams.integers(0, 999999999, 
                                          size=N_STREAMS)

        # inter-arrival distribution for unplanned cases
        self.arrival_dist_emerg = Exponential(emerg_iat, 
                                              random_seed=self.seeds[0])
        self.arrival_dist_ward = Exponential(ward_iat, 
                                             random_seed=self.seeds[1])
        self.arrival_dist_other = Exponential(other_iat, 
                                              random_seed=self.seeds[2])
        self.arrival_dist_AnE = Exponential(AnE_iat, 
                                            random_seed=self.seeds[3])
        self.arrival_dist_xray = Exponential(xray_iat , 
       ...
       #length of stay distribution for unplanned patients
        self.emerg_dist = Lognormal(emerg_mean_los, 
                                    emerg_std_los, 
                                    random_seed=self.seeds[6])
        self.ward_dist = Lognormal(ward_mean_los, 
                                   ward_std_los, 
                                         random_seed=self.seeds[7])
        self.other_dist = Lognormal(other_mean_los, 
                                    other_std_los, 
                                         random_seed=self.seeds[8])
        self.AnE_dist = Lognormal(AnE_mean_los, 
                                  AnE_std_los, 
                                         random_seed=self.seeds[9])
        self.xray_dist = Lognormal(xray_mean_los, 
                                   xray_std_los, 
                                         random_seed=self.seeds[10])
        self.elect_dist = Lognormal(elect_mean_los, 
                                    elect_std_los, 
...
class CCU_Patients:
    ...
    def patient_gets_cancelled(self):
        trace(f'{self.group["name"]}-patient {self.identifier} cancelled {self.env.now:.3f}')
    
    def patient_gets_admitted(self):
        trace(f'{self.group["name"]}-patient {self.identifier} admitted {self.env.now:.3f}; '
               + f'waiting time was {self.wait_time:.3f}')
    
    def patient_gets_discharged(self):
        trace(f'{self.group["name"]}-patient {self.identifier} discharged {self.env.now:.3f}')
...
class CCU_model:
    ...
    def init_resources(self):
        ...
        '''
        self.args.beds = simpy.PriorityResource(self.env, 
                                          capacity=self.args.CCU_beds)
    ...
    def arrivals_generator(self, group):
        ...
        # create a new minor patient and pass in env and args
            new_patient = MonitoredPatient(patient_count, self.env, self.args, group, self)
        ...
        if msg == 'patient_gets_admitted':
            ...
            if patient.group['type']=='unplanned':
               self.wait_for_CCU_bed += \
                (patient.wait_time - self.wait_for_CCU_bed) / n
    ...
    def run_summary_frame(self):
        ...
        df = pd.DataFrame({'1':{'CCU_bed_wait_time': CCU_bed_wait_time, 
...
class WarmupAuditor():
    ...
    def summary_frame(self):
        ...
        df = pd.DataFrame([self.wait_for_CCU_bed,
                           self.bed_util,
                           self.CCU_bed_queue
                           ]).T
...
def warmup_analysis(scenario, rc_period, n_reps=DEFAULT_N_REPS,
    ...
    # index as obs number
    metrics['CCU_bed_wait_time'].index = np.arange(1, 
                                               len(metrics['CCU_bed_wait_time'])+1)
